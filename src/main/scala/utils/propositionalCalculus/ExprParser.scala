...
class ExprParser {
  ...
  class ParboiledParser(val input: ParserInput) extends Parser {
    ...
    private def predicate: Rule1[Expr] = rule { term ~ "=" ~ term ~> ((a: Term, b: Term) => Predicate("=", a, b)) |
      capture(oneOrMore(CharPredicate.UpperAlpha)) ~ optional("(" ~ zeroOrMore(term).separatedBy(",") ~ ")") ~>
        ((a: String, b:Option[Seq[Term]]) => if (b.isEmpty) Predicate(a) else Predicate(a, b.get: _*)) | term }
    def term: Rule1[Term] =
