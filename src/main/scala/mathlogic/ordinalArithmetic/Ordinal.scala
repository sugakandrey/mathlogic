...
case class *(lhs: Ordinal, rhs: Ordinal) extends Ordinal { override def toString: String = s"$lhs*$rhs" }
case class +(lhs: Ordinal, rhs: Ordinal) extends Ordinal { override def toString: String = s"$lhs+$rhs" }
case class ^(lhs: Ordinal, rhs: Ordinal) extends Ordinal { override def toString: String = s"$lhs^$rhs" }
case class W() extends Ordinal { override def toString: String = "w" }
case class Nat(i: Int) extends Ordinal { override def toString: String = i.toString }
...
sealed trait CNF_T extends Ordered[CNF_T] {
  ...
  override def toString: String = this match {
    case CNF(_) => s"w" +
                   (if (firstExp > 1) s"^($firstExp)" else "") +
                   (if (firstCoeff > 1) s"*$firstCoeff" else "") +
                   (if (rest > 0) s"+$rest" else "")
...
case class Atom(i: BInt) extends CNF_T

object CNF_T {
  /** Nat number to possibly infinite power */
  def exp1(a: Atom, pow: CNF_T): CNF_T = pow match {
    case _ if pow.firstExp == one => val atom = pow.firstExp match { case a@Atom(_) => a }
                                     (pow.firstCoeff, Atom(a.i.exp(atom.i))) ::: 0
    case _ if (pow.rest match {
      case Atom(_) => true
      case _ => false
  }) => val atom = pow.rest match { case a@Atom(_) => a }
