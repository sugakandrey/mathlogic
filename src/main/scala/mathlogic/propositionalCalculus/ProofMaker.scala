...
import mathlogic.propositionalCalculus.Deduction._
import mathlogic.propositionalCalculus.{ProvingUtils => Proofs}
...
object ProofMaker {
  ...
  def provePrimitive(ctx: Vars, e: Expr): Array[Expr] = {
    ...
    e match {
      ...
      case Negation(Disj(a, b)) if !evalInCtx(a) && !evalInCtx(b) => provePrimitive(ctx, Negation(b)) ++
        provePrimitive(ctx, Negation(a)) ++ Proofs.negDisjunction(a, b)
